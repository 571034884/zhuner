...
public class BannerIndicator extends LinearLayout {
    ...
    public BannerIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        dashGap = (int) array.getDimension(R.styleable.BannerIndicator_gap, 6);
        slider_width = (int) array.getDimension(R.styleable.BannerIndicator_slider_width, 24);
        slider_height = (int) array.getDimension(R.styleable.BannerIndicator_slider_height, 8);
...
private void setLarge(int position) {
    if (getChildAt(position) instanceof BannerItemView) {
        ...
        ValueAnimator alpha = ObjectAnimator.ofFloat(getChildAt(position), "alpha", 0.3f, 1f);
    ...
    private ValueAnimator getEnlarge(BannerItemView roundRectView) {
        return ObjectAnimator.ofFloat(roundRectView, "rectWidth", 0, getOffset(roundRectView));
    } //根据大小变化方向获取指示器大小偏移量
    ...
    private ValueAnimator getShrink(BannerItemView roundRectView) {
        return ObjectAnimator.ofFloat(roundRectView, "rectWidth", getOffset(roundRectView), 0);
    }
    //缩小动画同事伴随透明度变化
    public void setSmall(int small) {
        if (getChildAt(small) instanceof BannerItemView) {
            AnimatorSet set = new AnimatorSet();
            ValueAnimator alpha = ObjectAnimator.ofFloat(getChildAt(position), "alpha", 1, 0.3f);
