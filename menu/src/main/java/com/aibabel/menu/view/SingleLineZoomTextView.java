...
public class SingleLineZoomTextView extends TextView {
    ...
    private void refitText(String text, int textWidth,int textHeight) {
        if (textWidth > 0) {
            ...
            int drawHei = 0;//drawableLeft，Right，Top，Buttom 所有图片的宽
            Drawable[] draws = getCompoundDrawables();
            for (int i = 0; i < draws.length; i++) {
                if(draws[i]!= null){
                    drawWid += draws[i].getBounds().width();
                    drawHei += draws[i].getBounds().height();
                }
            }
            //获得当前TextView的有效宽度
            int availableWidth = textWidth - this.getPaddingLeft()
                    - this.getPaddingRight()- getCompoundDrawablePadding()- drawWid;
            int availableHeight = textHeight - this.getPaddingTop()
                    - this.getPaddingBottom()- getCompoundDrawablePadding()- drawHei;
            //所有字符所占像素宽度
            float textWidths = getTextLength(mTextSize, text);
            while(textWidths > availableWidth||mTextSize > textHeight){
                mPaint.setTextSize(--mTextSize);//这里传入的单位是 px
                textWidths = getTextLength(mTextSize, text);
            }
//            Log.d("1111111111","文本大小："+mTextSize+"");
//            Log.d("1111111111","文本框高度："+textHeight+"");
