...
import com.aibabel.scenic.utils.ThreadPoolUtil;
...
public class MusicService extends Service implements MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, OnAudioFocusChangeListener {
    ...
    private static List<MusicBean> mMusic_list = new ArrayList<>();
...
//            mPosition = SharePrefUtil.getInt(getApplicationContext(), "music_current_position", 0);
...
//        notificationManager.cancel(Constants.NOTIFICATION_CEDE);
    ...
    private void sentPreparedMessageToMain() {
        ...
        mMessage.arg2 = mPlayer.getDuration();
    ...
    private void play(int position) {
        ...
        if (mPlayer != null && mMusic_list.size() > 0) {
            mPlayer.reset();
            try {
                mPlayer.setDataSource(mMusic_list.get(position).getAudioUrl());
        ...
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                ...
                case Constants.ACTION_NEW:
                    Log.i(TAG, "onReceive: ACTION_NEW");
                    cancelMusic();
                    mMusic_list = intent.getParcelableArrayListExtra("music_list");
                    break;
                case Constants.ACTION_ADD:
                    Log.i(TAG, "onReceive: ACTION_ADD");
                    List<MusicBean> list = intent.getParcelableArrayListExtra("music_list");
                    mMusic_list.addAll(list);
                    Log.i(TAG, "mMusic——list:" + mMusic_list.size());
                    break;
    ...
    public static void addData(List<MusicBean> list) {
        mMusic_list.addAll(list);
    }

    public static void setNewData(List<MusicBean> list) {
        if (mPlayer != null) {
            mPlayer.stop();
            mPlayer.release();
            mPlayer = null;
