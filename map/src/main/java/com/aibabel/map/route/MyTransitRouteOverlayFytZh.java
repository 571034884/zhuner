 ...
 */
public class MyTransitRouteOverlayFytZh extends OverlayManager {
    ...
    private BitmapDescriptor mRedTexture1 = BitmapDescriptorFactory.fromAsset("ic_route_center_ic.png");
    List<BitmapDescriptor> textureList1 = new ArrayList<>();
    List<Integer> textureIndexs1 = new ArrayList<>();

    private TransitRoutesZhBean routes = null;
     ...
     */
    public MyTransitRouteOverlayFytZh(BaiduMap baiduMap) {
        super(baiduMap);
    }
    ...
    public final List<OverlayOptions> getOverlayOptions() {
        ...
        textureIndexs.add(0);
        textureList1.add(mRedTexture1);
        textureIndexs1.add(0);

        List<OverlayOptions> overlayOptionses = new ArrayList<OverlayOptions>();

        List<List<TransitStepsZhBean>> stepsList = routes.getSteps();
        if (stepsList != null && stepsList.size() >0 ){
            for (int i = 0; i <stepsList.size();i++){
                List<TransitStepsZhBean> steps = stepsList.get(i);
                for (TransitStepsZhBean step : steps){
                    /**
                     * 3 公交或者地铁
                     *  ----detail---type---0公交
                     *  ----detail---type---1地铁
                    ...
                    if (type == 5){
                        ...
                        TransitVehicleZhBean vehicle = step.getVehicle();
                        int detailType = vehicle.getType();
                        if (detailType == 0){
                            overlayOptionses.add((new MarkerOptions())
                                    .position(new LatLng(step.getStart_location().getLat(),step.getStart_location().getLng()))
                                    .anchor(0.5f, 0.5f).zIndex(10)
     ...
     *            路线数据
     */
    public void setData(TransitRoutesZhBean routes, String origin, String destination) {
