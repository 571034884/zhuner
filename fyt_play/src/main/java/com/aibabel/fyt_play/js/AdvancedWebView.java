...
public class AdvancedWebView extends WebView implements JavascriptInterface {
	 ...
	 *            whether a correct MIME type is required for file uploads or `application/octet-stream` is acceptable
	 * @return whether file uploads can be used
	 */
	 ...
	 * @param fromUrl
	 *            the URL of the file to download, e.g. the one from `AdvancedWebView.onDownloadRequested(...)`
	 * @param toFilename
	 *            the name of the destination file where the download should be saved, e.g. `myImage.jpg`
	...
	public static class Browsers {
		 ...
		 *            whether to switch to the browser `Activity` without a transition
		 */
	...
	public void jsInterface(String json) {
		JsClickInfo jsc = new Gson().fromJson(json, JsClickInfo.class);
		if (jsc == null)
			return;
		jsc.init();
		// TODO 公用js事件 在这里处理
		JSClickListener.onJSClick(jsc);// 回调实现该webview的方法，实现独立功能独立处理
	}
