...
import com.aibabel.tucao.beans.PaymentBean;
import com.aibabel.tucao.utils.CommonUtils;
import com.aibabel.tucao.utils.FastJsonUtil;
...
//                response.getException()
     ...
     * @param mCallback 回调
     */
    public static <T> void post(final Context context, final String method, Map<String, Object> param, final Class<T> cls, final BaseCallback mCallback) {

        PostRequest<String> getRequest = OkGo.<String>post(Constans.HOST_XW + Constans.METHOD_GROUP_XW + method).tag(method);

        for (Map.Entry<String, Object> entry : param.entrySet()) {
            if (entry.getValue() instanceof String)
                getRequest.params(entry.getKey(), (String) entry.getValue());
            else if (entry.getValue() instanceof File) {
                getRequest.params(entry.getKey(), (File) entry.getValue());
            }
        }
...
//                response.getException()
    ...
    public static <T> void post(final Context context, final String method, JSONObject json, final Class<PaymentBean> cls, final BaseCallback mCallback) {
        ...
        PostRequest<String> postRequest = OkGo.<String>post(Constans.HOST_XW + Constans.METHOD_GROUP_XW + method).tag(method);
